...
class CVLoDRules:
    ...
    def villa_can_open_hybrid_rose_door(self, state: CollectionState) -> bool:
        """Rose Garden Key if the Villa state is specifically Hybrid. Always True if Reinhardt/Carrie or Cornell."""
    ...
    def villa_can_open_thorn_fence(self, state: CollectionState) -> bool:
        ...
        if self.villa_state == VillaState.option_reinhardt_carrie:
            return True
        return state.has(item_names.quest_key_thorn, self.player)
